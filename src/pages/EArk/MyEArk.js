...
import { ReactComponent as NarkEmpty } from '../../assets/svg/swapLogo.svg'
import Copy from '../../components/AccountDetails/Copy'
import { useMyEArk, useMyJoinedEArk } from '../../state/eark/hooks'
...
export default function MyEArk({ history }) {
  const { account } = useActiveWeb3React()
  const circle = useMyEArk()
  const jointedCircle = useMyJoinedEArk()
    ...
    <>
      <Wrapper>
        <AutoColumn gap="20px">
          {circle.name && (
            <AppBody>
              <CloseIcon style={{ top: 12 }} onClick={() => history.push('/eark')} />
              <ColumnCenter style={{ width: 463 }}>
                <TYPE.largeHeader>My EArk</TYPE.largeHeader>
                <CloseIcon />
              </ColumnCenter>
              <TYPE.main marginTop="44px">My EArk name</TYPE.main>
              <InputPanel>
                <Input>
                  {circle && circle.name}{' '}
          ...
          {!circle.name && !jointedCircle.name && (
            <AutoColumn gap="lg">
              <NarkEmpty />
              <ColumnCenter>
                <TYPE.largeHeader color="#2C2C2C">{`You don't have NArk`}</TYPE.largeHeader>
              </ColumnCenter>
              <ButtonPrimary
                padding="16px 16px"
                width="100%"
