...
import { useUserQueryRewards, useUserRewardTime } from '../../hooks/useArkSwap'
      ...
      {!attempting && (
        <ContentWrapper gap="lg">
          <ModalUpper>
            ...
            <CardSection gap="md">
              <RowBetween>
                <TYPE.white fontWeight={500}>{t('claimARK')}</TYPE.white>
                <CloseIcon onClick={wrappedOnDismiss} style={{ zIndex: 99 }} stroke="white" />
              </RowBetween>
              <TYPE.white fontWeight={700} fontSize={36}>
                {unclaimedAmount?.toFixed(2, { groupSeparator: ',' } ?? '-')} ARK
              </TYPE.white>
          ...
          <AutoColumn gap="md" style={{ padding: '1rem', paddingTop: '0' }} justify="center">
            ...
            {/*  Enter an address to trigger a ARK claim. If the address has any claimable ARK it will be sent to them on*/}
            {/*  submission.*/}
            {/*</TYPE.subHeader>*/}
            ...
            <AutoColumn gap="12px" justify={'center'}>
              ...
              {!claimConfirmed && (
                <Text fontSize={36} color={'#ff007a'} fontWeight={800}>
                  {unclaimedAmount?.toFixed(0, { groupSeparator: ',' } ?? '-')} ARK
                </Text>
              )}
              {account && (
                <TYPE.largeHeader fontWeight={600} color="black">
