...
import { useInviteContract, useMerkleDistributorContract } from '../../hooks/useContract'
...
export function useGetRewardsCallback(
  account: string | null | undefined
): {
  rewardCallback: () => Promise<string>
} {
  // get claim data for this account
  const { library, chainId } = useActiveWeb3React()
  // const claimData = useUserClaimData(account)
  ...
  const addTransaction = useTransactionAdder()
  // const distributorContract = useMerkleDistributorContract()
  const distributorContract = useInviteContract()
  // console.log('distributorContract', distributorContract)
  const rewardCallback = async function() {
    if (!account || !library || !chainId || !distributorContract) return

    const args: never[] = []

    return distributorContract.estimateGas['getRewards'](...args, {}).then(estimatedGasLimit => {
      return distributorContract
        .getRewards(...args, { value: null, gasLimit: calculateGasMargin(estimatedGasLimit) })
        .then((response: TransactionResponse) => {
          addTransaction(response, {
            summary: `Claimed ${unClaimedAmount?.toSignificant(4)} ARK`,
            claim: { recipient: account }
          })
          return response.hash
        })
